...
- [#86](https://github.com/everquint/frontend-skills/pull/86) [`f8d0c15`](https://github.com/everquint/frontend-skills/commit/f8d0c159f34f5fd3ff2de91503d73c18865fe57f) Thanks [@gokulsgr](https://github.com/gokulsgr)! - Points the 2.14.0 rule-count step at the right measurement.
...
- [#84](https://github.com/everquint/frontend-skills/pull/84) [`3482260`](https://github.com/everquint/frontend-skills/commit/3482260e44373afb14bd496e54d87fc2c7147a2d) Thanks [@gokulsgr](https://github.com/gokulsgr)! - Corrects the 2.14.0 migration step for the oxlint rule-count constant.
...
- [#82](https://github.com/everquint/frontend-skills/pull/82) [`3421b47`](https://github.com/everquint/frontend-skills/commit/3421b471bf21864e88cc73b1d5bcf843c27543bd) Thanks [@gokulsgr](https://github.com/gokulsgr)! - Kebab-case filenames are now enforced by the linter, not only by review and a whole-repo scan.
...
- [#79](https://github.com/everquint/frontend-skills/pull/79) [`cd2ee12`](https://github.com/everquint/frontend-skills/commit/cd2ee12153fbaf66101de91a2af78a0ec5520fad) Thanks [@gokulsgr](https://github.com/gokulsgr)! - Coverage enforcement changes shape. The gate is now **diff coverage** — `diff-cover` on the lines a
...
- [#77](https://github.com/everquint/frontend-skills/pull/77) [`d954369`](https://github.com/everquint/frontend-skills/commit/d95436978e341bd6152641acd385cf6dd39a4d00) Thanks [@gokulsgr](https://github.com/gokulsgr)! - `scripts/format-changelog.mjs` now ships with tests, and this repo can run them. The script runs
...
- [#75](https://github.com/everquint/frontend-skills/pull/75) [`137e015`](https://github.com/everquint/frontend-skills/commit/137e01592876b841dac99a1e5894128c40b27fd7) Thanks [@gokulsgr](https://github.com/gokulsgr)! - Reverses the v2.11.0 ruling that an app keeps no `CHANGELOG.md`. The changelog mechanism is now
...
- [#73](https://github.com/everquint/frontend-skills/pull/73) [`0df4d64`](https://github.com/everquint/frontend-skills/commit/0df4d645b68ac2586de5d2fcd061557d84aece1c) Thanks [@gokulsgr](https://github.com/gokulsgr)! - Apps now have a stated way to keep a changelog. v2.7.0 made changesets library-only but left apps with nothing; the standard now names the industry-normal alternative: tag each deploy and use GitHub's "Generate release notes", which lists every PR merged since the previous tag. The starter ships `.github/release.yml` to group those notes (Features / Fixes / Performance / Dependencies, then an unlabelled catch-all — order matters, GitHub files each PR under the first match); `skip-changelog` excludes a PR. init-greenfield now says which of the two changelog mechanisms to delete. Nothing to maintain by hand, no CHANGELOG merge conflicts. Libraries ignore the file — their CHANGELOG.md stays generated from changesets by the release job. Migration 2.11.0.

## [2.10.1] - 2026-08-07

### Fixed

- [#71](https://github.com/everquint/frontend-skills/pull/71) [`2afa8c0`](https://github.com/everquint/frontend-skills/commit/2afa8c0dfdbeabfe5e9e82ae24e1730bfae736c1) Thanks [@gokulsgr](https://github.com/gokulsgr)! - commitlint no longer red-gates dependency-bot PRs. @commitlint/config-conventional caps body lines at 100 characters, and Dependabot/Renovate bodies are long release-notes URLs, so a repo with a required commit-message check could not merge any bot PR — measured on a consumer repo's first Dependabot run: 4 of 5 PRs failed on the body cap alone, with lint, tests and build all green. The starter config now turns off `body-max-line-length` — deliberately narrower than commitlint`s `ignores`, which would exempt a matched commit from every rule; body readability is judged in /pre-pr step 8 instead. A human pasting a stack trace or URL was hitting the same wall. Migration 2.10.1 re-pulls the config.

## [2.10.0] - 2026-08-07

### Added

- [`4e169e2`](https://github.com/everquint/frontend-skills/commit/4e169e2916d8018822133f268cade00fed8ebdb6) Thanks [@gokulsgr](https://github.com/gokulsgr)! - /pre-pr gains step 8, commit granularity — field feedback: an agent session delivered a whole task as one big commit with one long description. The gate now reads `git log origin/HEAD..HEAD` and fails when a single commit mixes unrelated concerns (this standard merges with merge commits, so every commit is permanent history), when wip/typo noise commits should be squashed, or when the message is an unreadable inventory instead of the one change and why. Migration 2.10.0: re-pull pre-pr.md.

## [2.9.0] - 2026-08-06

### Added

- [`0e53dc3`](https://github.com/everquint/frontend-skills/commit/0e53dc31f1c6a042801d0b94d488c529c84b03fc) Thanks [@gokulsgr](https://github.com/gokulsgr)! - Branch names are now machine-enforced, closing a measured field failure: given a Linear issue, a session adopted Linear's suggested `<username>/<id>-<full-title>` branch name wholesale instead of the standard's `<type>/<ticket>-<short-slug>`. branch-guard now blocks `git commit` on a branch whose name is outside the format (types from the commit-type table; the tracker ID keeps its case; `CLAUDE_BRANCH_GUARD_ALLOW=1` remains the explicit override), the workflow skill's Branch naming section says outright that a tracker's suggested name is not the format, and migration 2.8.0 tells existing repos to re-copy the hook.

## [2.8.0] - 2026-08-06

### Added

- [#65](https://github.com/everquint/frontend-skills/pull/65) [`10d9b8e`](https://github.com/everquint/frontend-skills/commit/10d9b8efcbfb1693de96f3cb0f925f8496dc0587) Thanks [@gokulsgr](https://github.com/gokulsgr)! - feat(design-system): OKLCH token values and a machine-checked contrast gate
...
- [#63](https://github.com/everquint/frontend-skills/pull/63) [`453b9dc`](https://github.com/everquint/frontend-skills/commit/453b9dc6cf85c206772481ac33a4c0d99cfcbd44) Thanks [@gokulsgr](https://github.com/gokulsgr)! - The release requirement is now scoped by consumer, matching industry practice: a library — code other repos install and upgrade — requires the full machinery (Changesets, semver, the generated CHANGELOG.md, the release job); an app — deployed to users, upgraded by nobody — may skip it all, with the merged PRs as its history and release notes living with its deploys. An app that opts in follows the library rules. hygiene.md notes that the three starter release pieces are removed together when an app opts out, so a half-installed release job cannot linger.

## [2.6.0] - 2026-08-06

### Added

- [#61](https://github.com/everquint/frontend-skills/pull/61) [`a817110`](https://github.com/everquint/frontend-skills/commit/a817110c3def71201303424bc12096f51f5828ec) Thanks [@gokulsgr](https://github.com/gokulsgr)! - feat(design-system): add the eq-design-system skill
...
[Unreleased]: https://github.com/everquint/frontend-skills/compare/v2.14.2...HEAD
[2.14.2]: https://github.com/everquint/frontend-skills/compare/v2.14.1...v2.14.2
[2.14.1]: https://github.com/everquint/frontend-skills/compare/v2.14.0...v2.14.1
[2.14.0]: https://github.com/everquint/frontend-skills/compare/v2.13.0...v2.14.0
[2.13.0]: https://github.com/everquint/frontend-skills/compare/v2.12.1...v2.13.0
[2.12.1]: https://github.com/everquint/frontend-skills/compare/v2.12.0...v2.12.1
[2.12.0]: https://github.com/everquint/frontend-skills/compare/v2.11.0...v2.12.0
[2.11.0]: https://github.com/everquint/frontend-skills/compare/v2.10.1...v2.11.0
[2.10.1]: https://github.com/everquint/frontend-skills/compare/v2.10.0...v2.10.1
[2.10.0]: https://github.com/everquint/frontend-skills/compare/v2.9.0...v2.10.0
[2.9.0]: https://github.com/everquint/frontend-skills/compare/v2.8.0...v2.9.0
[2.8.0]: https://github.com/everquint/frontend-skills/compare/v2.7.0...v2.8.0
[2.7.0]: https://github.com/everquint/frontend-skills/compare/v2.6.0...v2.7.0
[2.6.0]: https://github.com/everquint/frontend-skills/compare/v2.5.0...v2.6.0
[2.5.0]: https://github.com/everquint/frontend-skills/compare/v2.4.1...v2.5.0
[2.4.1]: https://github.com/everquint/frontend-skills/compare/v2.4.0...v2.4.1
