...
// EXACTLY the preamble lines the output section emits, trimmed — the parser strips these wherever
// it meets them (see the loop), so the two lists must move together or old preambles survive.
const PREAMBLE_LINES = new Set([
...
for (const line of lines) {
    ...
    // The script's own preamble lines are dropped WHEREVER they appear, not only above the first
    // version: `changeset version` prepends its new `## x.y.z` heading directly under the H1, which
    // pushes the previous run's preamble INSIDE that version's body — retaining it there duplicated
    // the preamble between entries, compounding once per release (measured: three copies by 2.3.0).
    if (PREAMBLE_LINES.has(line.trim())) continue;
    if (current === null) continue;   // anything else above the first version — regenerated
