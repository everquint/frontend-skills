...
const MIGRATIONS = {
    ...
    '2.14.0': [
        ...
        'Then re-measure `EXPECTED_OXLINT_RULES` in .github/workflows/ci.yml using the command in the comment block directly above it — `oxlint -c .oxlintrc.strict.json --type-aware -f json src`, read `number_of_rules` — and use YOUR number. Do not paste one from here and do not use measure-rules.mjs: both report the STARTER\'s count (227), while a repo whose config adds rules of its own is higher. The constant is a floor, so writing a lower number silently weakens the gate. A repo that already enabled the unicorn plugin sees no change at all.',
