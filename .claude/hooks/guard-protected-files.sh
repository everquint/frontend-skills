...
# THIS REPO'S gate files — the standard repo itself, not a consumer app, so the list differs from
# the starter's: no lint/tsconfig gates here; the gate is the validators, the release pipeline, and
# the hooks. skills/ content is deliberately NOT listed — editing the skills is this repo's normal
# work, and the validators gate it.
case "$rel" in
    scripts/validate-skills.mjs|scripts/validate-claims.mjs)
        deny 'the validator the pre-commit hook and CI run — weakening it turns doc drift invisible' ;;
    scripts/sync-standard-version.mjs)
        deny 'the release-time version sync — break it and STANDARD_VERSION silently drifts from package.json' ;;
    .changeset/config.json)
        deny 'the release configuration — privatePackages.tag:false makes releases report success while tagging nothing' ;;
    .husky/*)
        deny 'a git hook' ;;
    .github/workflows/*)
        deny 'the CI gate' ;;
    ...
    .git-blame-ignore-revs)
        deny 'the blame attribution for mechanical rewraps — entries are appended with their commit, never edited' ;;
